...
- **Sentry crash reporting:** `lib/monitoring/sentry.ts` — `EXPO_PUBLIC_SENTRY_DSN` ile init (DEV'de disabled), `tracesSampleRate: 0.1`, `sendDefaultPii: false`. `beforeSend`/`beforeBreadcrumb` PII sanitize ediyor: `token|secret|auth|password|key|cookie|session` regex'iyle eşleşen anahtarlar `[redacted]`, string'ler 500 char'a kırpılır, derinlik 4'te `[truncated]`, user objesi sadece `id` ile bırakılır. `app/_layout.tsx` modül seviyesinde `initSentry()` çağırıyor, `ErrorBoundary onError={reportError}`.
