...
- `components/portfolio/TxModal.tsx` — varlık ekleme modalı. **Geçmiş Fiyat Asistanı (Phase 1):** Alış (`side==="buy"`) + geçmiş tarih (`datePreset!=="today"`) + history-supported sembol durumunda tarih bölümünün altında sade bir kart gösterir: o günün piyasa fiyatı + bugüne göre %değişim + "Kullan" butonu. Kart `colors.secondary` arka planlı, mevcut "Güncel Piyasa" kartı stilini takip eder; AI/öneri görseli yok.
- `hooks/useHistoricalPriceAt.ts` — `(code, date|null) → {loading, supported, point, error}`. Hook girişi null olduğunda fetch tetiklemez (modal kapalı/satış/bugün durumlarında pasif). `pickRangeForDate` ile en küçük uygun range seçilir (1H/1A/3A/1Y/3Y/5Y, +5g buffer). 6 saat in-memory cache. `reqId` her effect başında bump edilir → cache-hit ve fetch yollarında stale response overwrite koruması var.
- `lib/api/historyApi.ts` helpers: `pickRangeForDate(date)` (en küçük range seçimi), `findClosestPoint(points, target, ±5g)` (hafta sonu/tatil toleransı).
...
- **TypeScript strict:** `noImplicitOverride: true` + `noUncheckedIndexedAccess: true` eklendi. Index/array erişimleri `T | undefined` döner; tüm code path'ler nullable check veya `!` non-null assertion ile temizlendi (0 TS hatası). ErrorBoundary `state`/`componentDidCatch`/`render` `override` ile işaretlendi.
...
- **Test altyapısı:** Jest projects: `utils` (ts-jest, node) + `components` (jest-expo + RNTL). `pnpm --filter @workspace/doviz-altin test`. 25 test geçiyor — utility: `__tests__/portfolioCalc.test.ts` (FIFO P/L, bucket, available), `__tests__/alertEvaluator.test.ts` (4 alarm tipi + canFire), `__tests__/alertFormat.test.ts` (TR metin); component: `__tests__/ErrorBoundary.test.tsx` (happy path + error capture + onError callback).
- **Sentry crash reporting:** `lib/monitoring/sentry.ts` — `EXPO_PUBLIC_SENTRY_DSN` ile init, `EXPO_PUBLIC_APP_ENV` (development/preview/production) ile `environment` etiketi + sample-rate; `enabled: APP_ENV !== "development"`, `sendDefaultPii: false`. `beforeSend`/`beforeBreadcrumb` PII sanitize ediyor: `token|secret|auth|password|key|cookie|session` regex'iyle eşleşen anahtarlar `[redacted]`, string'ler 500 char'a kırpılır, derinlik 4'te `[truncated]`, user objesi sadece `id` ile bırakılır. `app/_layout.tsx` modül seviyesinde `initSentry()` çağırıyor, `ErrorBoundary onError={reportError}`.
- **Sentry build wiring:** `app.json` plugins'e `@sentry/react-native/expo` (org/project env'den okunur) eklendi. `eas.json` üç profile env block'u: development → `EXPO_PUBLIC_APP_ENV=development`; preview/production → `EXPO_PUBLIC_APP_ENV` + `EXPO_PUBLIC_SENTRY_DSN` + `SENTRY_AUTH_TOKEN/ORG/PROJECT` (build-time source-map upload için).
- **Format helpers + tests:** `lib/utils/format.ts` (`formatPrice`/`formatPercent`/`formatTimeAgo`/`formatDate`/`formatChange` — TR-locale, ondalık duyarlılığı dinamik). `__tests__/format.test.ts` 5 helper + symbolDescriptions (formatSymbolName / getCategoryTitle/Subtitle / getSymbolDescription) için kapsamlı test — toplam 45/45 yeşil (önceki 25 + 20 yeni).
- **Push token self-healing (client):** `lib/notifications/tokenValidator.ts → validateAndRefreshToken()` AppContext açılışında setupPushAndRegister sonrası çağrılır; AsyncStorage cache'lenmiş token'la güncelle karşılaştırır → unchanged | rotated (re-register) | deleted (kullanıcı izinleri iptal etti → `apiDeleteToken` ile server'dan sil) | skipped. Server tarafı: yeni `deleteToken` HTTP endpoint `tokens/{deviceId}` Firestore doc'unu siler. `cleanupStaleTokens` cron'unu (haftalık, 30g+ TTL) tamamlayıcı: cron'u beklemeden anlık geçersiz token'ları temizler.
