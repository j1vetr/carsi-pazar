...
export function CurrencyHomeV2Dark() {
  return (
    <PhoneShell theme="dark">
      <TopBar theme="dark" />
      <Segments theme="dark" items={["Tümü", "Favoriler", "Pariteler", "Çapraz"]} active={0} />
      <SectionTitle theme="dark" title="Döviz Kurları" meta="CANLI" />
      <TableHeader theme="dark" />
      <div>
        {ROWS.map((r, i) => (
          <Row key={r.code} theme="dark" row={r} last={i === ROWS.length - 1} />
        ))}
      </div>
      <div style={{ height: 24 }} />
    </PhoneShell>
...
export default CurrencyHomeV2Dark;
