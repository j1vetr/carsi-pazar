...
export function GoldHomeV2Light() {
  return (
    <PhoneShell theme="light">
      <TopBar theme="light" />
      <Segments theme="light" items={["Hepsi", "Sarrafiye", "Külçe", "Madenler"]} active={0} />
      <SectionTitle theme="light" title="Altın Fiyatları" meta="KAPALIÇARŞI" />
      <TableHeader theme="light" />
      <div>
        {ROWS.map((r, i) => (
          <Row key={`${r.code}-${i}`} theme="light" row={r} last={i === ROWS.length - 1} />
        ))}
      </div>
      <div style={{ height: 24 }} />
    </PhoneShell>
...
export default GoldHomeV2Light;
